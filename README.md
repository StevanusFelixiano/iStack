...
* We were also surprised by Apple's strict privacy and battery policies: `WCSession` messages from iPhone to Apple Watch are blocked if the Watch screen is off, and background execution is heavily restricted.
...
* We initially built a watchOS app using `WKExtendedRuntimeSession`, but after testing background execution, we rebuilt the architecture using `HKWorkoutSession` (`.mindAndBody`) to continuously query heart rate (`HKAnchoredObjectQuery`) and track user movement using Core Motion (`CMMotionActivityManager`).
...
**We considered:**
Relying exclusively on Heart Rate to detect stress, integrating via HomeKit for a more "native" Apple ecosystem feel, and using `WKExtendedRuntimeSession` to run the Watch app in the background.
...
* **WKExtendedRuntimeSession:** Dropped because watchOS aggressively suspends it in the background. This broke our `WCSession` connection, making it impossible to send immediate BLE triggers to the ESP32 when the screen was off.
...
We hit a major wall with watchOS background execution and `WCSession`. When the iPhone sent a "Start Session" command to the Watch, it would silently fail and return *“Apple Watch is not reachable”* if the Watch screen was asleep. The system would also kill our sensor queries to save battery unless explicitly forced into a highly privileged state.

**How we worked around it (or how it changed our use case / mechanic):**
We pivoted completely to using an `HKWorkoutSession` (specifically the `.mindAndBody` activity type). This forced watchOS to prioritize the app, keeping the biometric sensors active and the `WCSession` communication channel awake even when the screen turned off. We also implemented a "Lazy Initialization" fix (`_ = ConnectivityManager.shared` on init) to ensure the Watch immediately handshakes with the iPhone upon opening.

## 🔄 The Revised Decision

**Final decision:**
Core Bluetooth (Primary) + HealthKit (via HKWorkoutSession) + Core Motion. 

**What changed since Section 1, and why:**
Our core stack remained the same, but the *logic* and *architecture* changed drastically. Instead of a passive background monitor, it became an interactive "Mindfulness / Tension Relief" session. By using a workout session, the Watch continuously detects potential tension -> iPhone triggers ESP32 via BLE -> LED turns on -> User punches the pillow -> MPU6050 detects impact magnitude -> iOS logs the recovery analytics. We successfully bridged a digital Apple framework state into a physical IoT tension-relief action without losing background connectivity.

## 📱 App Track Addendum

### About the Frameworks
**Does your use case genuinely need both frameworks working together, or could it work with just your main one?**
Yes, it genuinely needs all three. Without HealthKit, we can't detect the physiological trigger or maintain background execution. Without Core Motion, we get false positives from walking. Without Core Bluetooth, we just have a passive dashboard with no physical tension-relief mechanism (the smart pillow).
...
The app strictly needs `NSBluetoothAlwaysUsageDescription`, Health (Heart Rate, Resting Heart Rate, & Workout), and Motion access. If the user denies HealthKit, the app gracefully degrades: the Watch UI explicitly shows a "Need Sensor Access" state, and we provide a "Simulation Mode" (via manual UI buttons) to manually trigger a tension spike so the user can still experience punching the IoT pillow without biometric data.
